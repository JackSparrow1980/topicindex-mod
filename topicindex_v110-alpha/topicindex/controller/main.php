				...
				foreach ($fixedlist as $indexdata)
				{
					if ($realfilter == $indexdata['key'])
					{
						$this->template->assign_block_vars('block_list.block_topic_index', array(
							...
							'ITOPIC_FORUM'  => $indexdata['forum'], // Questa linea è fondamentale
							...
							'U_REMOVE_FROM' => append_sid("{$this->root_path}viewtopic.$this->php_ext", "f={$forum_id}&t={$indexdata['id']}&iremove=1"),
						));
						$nums++;
					}
				}
